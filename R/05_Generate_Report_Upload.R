...
## Long report from PowerBI
for (region in bureaux) {
  
  outputfile <-  here::here("out", paste0("QA_report_Generated_2_",
                                          region,
                                          "v_",
                                          format(Sys.Date(),
                                                 '%d %B %Y'), 
  ...
  rmarkdown::render(
    ## Report template
    here::here("R/Rmd","QA_report_Notebook_PBIDATA_2.Rmd"),
    ## geneated file
    output_file = outputfile,
    ## Report parameters
    params = list(region = region))
  ...
  ## upload the generated report to Sharepoint
  drv$upload_file(src = outputfile,
                  dest = paste0("02 Operations/04. RBM - COMPASS/07. Results Framework and M&E/05. Indicators/03. Core Indicator Quality Assurance/2022 Year-End Reporting/05_Results_PowerBI/QA_report_Generated_2_",
                                region,
                                "v_",
                                format(Sys.Date(),
                                       '%d %B %Y'), 
